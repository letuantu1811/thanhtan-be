...
class TreatmentFormService {
    async getInvoiceTemplateByMode(mode, payload) {
...
const mapToExaminationFormPrint = (payload) => {
    ...
    const discountMember = totalAmount
        ? (totalAmount * toNumber(payload.addedDiscountAmount)) / 100
        : 0;
    const totalAmountAfterDiscount =
        totalAmount - discountMember - toNumber(unMaskPrice(payload.discountAmount));
...
const calculateTotalAmount = (payload) => {
    ...
    const totalAmountServices =
        sumBy(services, (service) => toNumber(unMaskPrice(service.gia))) || 0;
...
const makeHtmlTableTotalProductsToRender = (payload) => {
    ...
    const amountProductAsDrugMoney = productAsDrugMoney.length
        ? toNumber(unMaskPrice(productAsDrugMoney[0].gia))
        : 0;
